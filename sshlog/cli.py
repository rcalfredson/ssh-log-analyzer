...
def main():
    ...
    p.add_argument("--log", action="append", required=True, help="Path to an auth log (repeatable).")
    p.add_argument("--threshold", type=int, default=5, help="Fails per IP to flag brute-force.")
    p.add_argument("--window", default="10m", help="Time window like 10m, 1h, 1d.")
    p.add_argument("--csv", help="Base CSV path (creates *_failed.csv and *_alerts.csv).")
